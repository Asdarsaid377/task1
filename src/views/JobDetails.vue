...
const handleSubmit = async (payload: Omit<IApplication, "id">) => {
	try {
		loading.value = true;
		const add = await applicationService.addApplication(payload);
		console.log(add.id);
		alertService.success("Job added with ID:", 2000);
	} catch (err) {
		console.error(err);
	} finally {
		loading.value = false;
	...
	<main class="flex-1 bg-white flex justify-center py-10 px-4 md:px-10">
		<div class="layout-content-container flex flex-col max-w-7xl flex-1 gap-8">
			...
			<div
				...
				<h2 class="text-slate-900 text-2xl font-bold leading-tight mb-2">
